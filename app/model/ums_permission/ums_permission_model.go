...
// arModel is a active record design model for table ums_permission operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of ums_permission.
	Table = "ums_permission"
	// Model is the model object of ums_permission.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table ums_permission.
	Columns = struct {
		Id          string //                                                      
        Pid         string // 父级权限id                                           
        Name        string // 名称                                                 
        Value       string // 权限值                                               
        Icon        string // 图标                                                 
        Type        string // 权限类型：0->目录；1->菜单；2->按钮（接口绑定权限）  
        Uri         string // 前端资源路径                                         
