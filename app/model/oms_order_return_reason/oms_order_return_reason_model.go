...
// arModel is a active record design model for table oms_order_return_reason operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of oms_order_return_reason.
	Table = "oms_order_return_reason"
	// Model is the model object of oms_order_return_reason.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table oms_order_return_reason.
	Columns = struct {
		Id          string //                           
        Name        string // 退货类型                  
        Sort        string //                           
        Status      string // 状态：0->不启用；1->启用  
