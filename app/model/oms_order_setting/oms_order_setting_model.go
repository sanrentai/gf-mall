...
// arModel is a active record design model for table oms_order_setting operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of oms_order_setting.
	Table = "oms_order_setting"
	// Model is the model object of oms_order_setting.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table oms_order_setting.
	Columns = struct {
		Id                   string //                                       
        FlashOrderOvertime   string // 秒杀订单超时关闭时间(分)              
        NormalOrderOvertime  string // 正常订单超时时间(分)                  
        ConfirmOvertime      string // 发货后自动确认收货时间（天）          
        FinishOvertime       string // 自动完成交易时间，不能申请售后（天）  
        CommentOvertime      string // 订单完成后自动好评时间（天）
	}{
		Id:                  "id",                     
        FlashOrderOvertime:  "flash_order_overtime",   
        NormalOrderOvertime: "normal_order_overtime",  
        ConfirmOvertime:     "confirm_overtime",       
        FinishOvertime:      "finish_overtime",        
        CommentOvertime:     "comment_overtime",
	}
)
