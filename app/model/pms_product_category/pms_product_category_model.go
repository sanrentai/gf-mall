...
// arModel is a active record design model for table pms_product_category operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of pms_product_category.
	Table = "pms_product_category"
	// Model is the model object of pms_product_category.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table pms_product_category.
	Columns = struct {
		Id            string //                                       
        ParentId      string // 上机分类的编号：0表示一级分类         
        Name          string //                                       
        Level         string // 分类级别：0->1级；1->2级              
        ProductCount  string //                                       
        ProductUnit   string //                                       
        NavStatus     string // 是否显示在导航栏：0->不显示；1->显示  
        ShowStatus    string // 显示状态：0->不显示；1->显示          
        Sort          string //                                       
        Icon          string // 图标                                  
        Keywords      string //                                       
