...
// arModel is a active record design model for table pms_product_attribute operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of pms_product_attribute.
	Table = "pms_product_attribute"
	// Model is the model object of pms_product_attribute.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table pms_product_attribute.
	Columns = struct {
		Id                          string //                                                          
        ProductAttributeCategoryId  string //                                                          
        Name                        string //                                                          
        SelectType                  string // 属性选择类型：0->唯一；1->单选；2->多选                  
        InputType                   string // 属性录入方式：0->手工录入；1->从列表中选取               
        InputList                   string // 可选值列表，以逗号隔开                                   
        Sort                        string // 排序字段：最高的可以单独上传图片                         
        FilterType                  string // 分类筛选样式：1->普通；1->颜色                           
        SearchType                  string // 检索类型；0->不需要进行检索；1->关键字检索；2->范围检索  
        RelatedStatus               string // 相同属性产品是否关联；0->不关联；1->关联                 
        HandAddStatus               string // 是否支持手动新增；0->不支持；1->支持                     
        Type                        string // 属性的类型；0->规格；1->参数
	}{
		Id:                         "id",                             
