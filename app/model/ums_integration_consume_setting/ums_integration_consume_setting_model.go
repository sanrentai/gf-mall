...
// arModel is a active record design model for table ums_integration_consume_setting operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of ums_integration_consume_setting.
	Table = "ums_integration_consume_setting"
	// Model is the model object of ums_integration_consume_setting.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table ums_integration_consume_setting.
	Columns = struct {
		Id                  string //                                           
        DeductionPerAmount  string // 每一元需要抵扣的积分数量                  
        MaxPercentPerOrder  string // 每笔订单最高抵用百分比                    
        UseUnit             string // 每次使用积分最小单位100                   
        CouponStatus        string // 是否可以和优惠券同用；0->不可以；1->可以
	}{
		Id:                 "id",                     
