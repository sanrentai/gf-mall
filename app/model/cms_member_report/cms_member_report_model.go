...
// arModel is a active record design model for table cms_member_report operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of cms_member_report.
	Table = "cms_member_report"
	// Model is the model object of cms_member_report.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table cms_member_report.
	Columns = struct {
		Id                string //                                                  
        ReportType        string // 举报类型：0->商品评价；1->话题内容；2->用户评论  
        ReportMemberName  string // 举报人                                           
        CreateTime        string //                                                  
        ReportObject      string //                                                  
        ReportStatus      string // 举报状态：0->未处理；1->已处理                   
        HandleStatus      string // 处理结果：0->无效；1->有效；2->恶意              
        Note              string //
	}{
		Id:               "id",                  
        ReportType:       "report_type",         
        ReportMemberName: "report_member_name",  
