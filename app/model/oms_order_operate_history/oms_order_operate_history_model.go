...
// arModel is a active record design model for table oms_order_operate_history operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of oms_order_operate_history.
	Table = "oms_order_operate_history"
	// Model is the model object of oms_order_operate_history.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table oms_order_operate_history.
	Columns = struct {
		Id           string //                                                                               
        OrderId      string // 订单id                                                                        
        OperateMan   string // 操作人：用户；系统；后台管理员                                                
        CreateTime   string // 操作时间                                                                      
        OrderStatus  string // 订单状态：0->待付款；1->待发货；2->已发货；3->已完成；4->已关闭；5->无效订单  
        Note         string // 备注
	}{
		Id:          "id",            
        OrderId:     "order_id",      
